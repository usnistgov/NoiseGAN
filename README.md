...
Packages in our python environment are documented by `conda_requirements.yaml` and `pip_requirements.txt` in the `experiment_resources/` subdirectory, which were created with the `conda list` and `pip freeze` commands, respectively.  
